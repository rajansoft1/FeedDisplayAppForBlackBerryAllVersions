...
import mypack.*;
import mypack.MyMovieStorage.MyActor;
...
public class StarMeter extends MainScreen implements FieldChangeListener
{
    Bitmap bmp;
    Bitmap la;
	...
	int hpos = 46;
	int vpos = 30;
	//LoadNextItems lni = new LoadNextItems(Bitmap.getBitmapResource("load5star.png"),Bitmap.getBitmapResource("load5star2.png"));
	...
	public StarMeter()
	{
		 ...
		 switch(scrwidth)
		 {
		  case 320 : bmp = Bitmap.getBitmapResource("320header2.png");
                     la = Bitmap.getBitmapResource("startitle320.png");
                     hpos = 46;
                     vpos = 30;
                     break;
		  case 480 : bmp = Bitmap.getBitmapResource("480header2.png");
                     la = Bitmap.getBitmapResource("startitle480.PNG");
                     hpos = 50;
                     vpos = 30;
                     break;
		  default : bmp = Bitmap.getBitmapResource("360header2.png");
		            la = Bitmap.getBitmapResource("startitle.png");
		            hpos = 46;
                    vpos = 30;
		            break;
		 }
	    ...
	    vm.setMargin(vpos, 0, 0, 0);
		 try{
		    ...
		    records = mms.fetchDataItems("A", "star-meter");
		...
		// thbmp = Bitmap.getBitmapResource("zelda.jpg");
		 ...
		 { tr = records.length;
			 if(tr >= 10)
			 {
		       for(int i = 0; i < 10; i++)
		       {
	             mm = mms.createMyActor(records[i]);
	           //  uti = new UrlToImage(mm.getStarImage(),"T");
	           //  thbmp = uti.getbitmap();
		         vm.add(new MyField(mm.getTitle(),mm.getStarImage(),mm.getRankThisWeek(),mm.getRankLastWeek(),mm.getStarRating()));
	...
	public void fieldChanged(Field field, int context) 
    {
     ...
     //   {
       ...
       // 	   {
        ...
        //	            uti = new UrlToImage(mm.getStarImage(),"T");
        //	            thbmp = uti.getbitmap();
        //		        vm.add(new MyField(mm.getTitle(),thbmp,mm.getRankThisWeek(),mm.getRankLastWeek(),mm.getStarRating()));
        		 //     vm.add(new MyField(title[i],thbmp,crank[i],prank[i],rate[i]));
        //		       }
        //		       vr+=5;
        //		       rr=tr-vr;
